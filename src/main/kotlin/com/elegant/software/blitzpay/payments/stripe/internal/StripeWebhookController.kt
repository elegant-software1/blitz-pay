    ...
    fun receive(
        ...
        when (event.type) {
            "payment_intent.succeeded" -> {
                log.info("stripe webhook settled paymentRequestId={} intentId={}", paymentRequestId, paymentIntent.id)
                paymentStatusUpdateGateway.settle(paymentRequestId, "stripe:${event.id}", occurredAt)
            }
            "payment_intent.payment_failed" -> {
                log.info("stripe webhook failed paymentRequestId={} intentId={}", paymentRequestId, paymentIntent.id)
