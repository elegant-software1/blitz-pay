...
class StripePaymentServiceTest {
    ...
    fun `rejects zero amount`() {
        StepVerifier.create(service.createIntent(0.0, "eur", credentials, merchantId, null, null))
    ...
    fun `rejects negative amount`() {
        StepVerifier.create(service.createIntent(-5.0, "eur", credentials, merchantId, null, null))
            .expectErrorMatches { it is IllegalArgumentException && it.message!!.contains("positive") }
            .verify()
    }
    ...
    fun `rejects NaN amount`() {
        StepVerifier.create(service.createIntent(Double.NaN, "eur", credentials, merchantId, null, null))
    ...
    fun `rejects infinite amount`() {
        StepVerifier.create(service.createIntent(Double.POSITIVE_INFINITY, "eur", credentials, merchantId, null, null))
            .expectErrorMatches { it is IllegalArgumentException }
            .verify()
    }
}
